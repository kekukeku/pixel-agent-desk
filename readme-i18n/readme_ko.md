...
2. **대시보드 실행**: [`Start.command`](Start.command)를 더블클릭합니다.
   ...
   - *권한 참고: macOS에서 `Install.command` 또는 `Start.command`를 열 수 없다고 표시되면 터미널에서 이 폴더로 이동한 뒤 `chmod +x Install.command Start.command`를 실행하세요.*
   - *Gatekeeper 참고: macOS가 실행을 차단하면 `.command` 파일을 우클릭하여 **열기**를 선택하세요.*
