...
2. **ダッシュボードの起動**：[`Start.command`](Start.command) をダブルクリックします。
   ...
   - *権限に関する注記：macOS が `Install.command` または `Start.command` を開けないと表示する場合は、ターミナルでこのフォルダに移動し、`chmod +x Install.command Start.command` を実行してください。*
   - *Gatekeeper 注記：macOS が実行をブロックする場合は、`.command` ファイルを右クリックして「開く」を選択してください。*
