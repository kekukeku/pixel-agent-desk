...
ENV_FILE="${PIXEL_AGENT_DESK_ENV_FILE:-$HOME/.pixel-agent-desk/reviewer.env}"
...
export PIXEL_AGENT_DESK_WATCHER_EXECUTION_MODE="${PIXEL_AGENT_DESK_WATCHER_EXECUTION_MODE:-active}"

if [[ "$REVIEWER_ENGINE" == "xai" && -z "${XAI_API_KEY:-}" ]]; then
  cat >&2 <<'EOF'
Missing XAI_API_KEY for REVIEWER_ENGINE=xai.

Create ~/.pixel-agent-desk/reviewer.env with:
...
Install Grok Build TUI or set GROK_BIN in ~/.pixel-agent-desk/reviewer.env
EOF
  exit 1
fi
...
if [[ -n "$EXISTING_WATCHER" ]]; then
  echo "Pixel Agent Desk watcher is already running (pid $EXISTING_WATCHER)."
...
echo "Starting Pixel Agent Desk watcher in $PIXEL_AGENT_DESK_WATCHER_EXECUTION_MODE mode"
python3 watcher.py
