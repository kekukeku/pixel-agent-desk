...
Characters appear only after an agent sends at least one lifecycle event. Start the agent CLI or IDE extension once (for example, open a Codex thread or send a Grok prompt) so PAD can detect and display it.
